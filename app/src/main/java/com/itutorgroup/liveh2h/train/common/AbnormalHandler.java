package com.itutorgroup.liveh2h.train.common;
...
import com.itutorgroup.liveh2h.train.BuildConfig;
...
public class AbnormalHandler implements UncaughtExceptionHandler {
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				Looper.prepare();
				Toast.makeText(mContext, R.string.crash, Toast.LENGTH_LONG).show();
				Looper.loop();
			}
		}.start();
		// 收集设备参数信息
		...
		try {
			if(BuildConfig.DEBUG){
				...
				if (Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED)) {
					String path = Environment.getExternalStorageDirectory() + "/mosai/";
...
//			else{
//				final SharedPreferences mPreferences = mContext.getSharedPreferences("crashinfo", 0);
//				int lastContentHascode = mPreferences.getString("content", "").hashCode();
//				if(lastContentHascode != content.hashCode()){
//					new Thread(){
//						public void run() {
//							try{
...
//										"smtp.163.com",
//										"25",
//										"mastercom2014@163.com",
//										"mastercom168",
//										new String[] { "813513532@qq.com" },
//										mContext.getResources().getString(
//												R.string.app_name)
//												+ "bug", content_email, null);
//								LogUtils.e("发送异常日志邮件成功...");
//								mPreferences.edit().putString("content", content)
//								   .putString("crashtime", crashtime)
//								   .commit();
//							} catch (Exception e) {
//								e.printStackTrace();
